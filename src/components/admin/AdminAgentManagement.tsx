...
import { UserPlus, Trash2, Phone, Mail, MessageSquare, Edit, Save, X, CheckCircle, MapPin, Star, Clock, Eye, XCircle } from "lucide-react";
...
const AdminAgentManagement = ({ users, userRoles, onRefresh, adminId }: AdminAgentManagementProps) => {
  ...
  const [createForm, setCreateForm] = useState({ email: "", password: "", full_name: "", phone: "", city: "", bio: "", experience_years: 0, specialization: "" });
  ...
  const fetchApplications = async () => {
    const { data } = await (supabase.from("agent_applications") as any).select("*").order("created_at", { ascending: false });
  ...
  const createAgent = async () => {
    ...
    try {
      ...
      if (authData.user) {
        ...
        await supabase.from("user_roles").insert({ user_id: authData.user.id, role: "agent" as any, assigned_by: adminId });
        ...
        try {
          await supabase.functions.invoke("agent-approval-email", {
            body: { to: createForm.email, agentName: createForm.full_name, agentId, password, loginUrl: `${window.location.origin}/auth` },
          });
        } catch { /* Email notification optional */ }
      }

      toast({ title: "✅ Agent created!", description: `Agent ID: ${agentId}` });
      setCreateForm({ email: "", password: "", full_name: "", phone: "", city: "", bio: "", experience_years: 0, specialization: "" });
  ...
  const approveApplication = async (app: any) => {
    ...
    try {
      ...
      await supabase.from("user_roles").insert({ user_id: app.user_id, role: "agent" as any, assigned_by: adminId });
      ...
      await supabase.auth.admin?.updateUserById?.(app.user_id, { password });

      // Log activity
      await supabase.from("admin_activity_log").insert({
        admin_id: adminId, action: "approve_agent_application", entity_type: "user", entity_id: app.user_id,
        details: { email: app.email, name: app.full_name, agent_id: agentId },
      });

      // Send notification
      await supabase.from("notifications").insert({
        user_id: app.user_id, title: "🎉 Agent Application Approved!",
        message: `Congratulations! Your agent application has been approved. Agent ID: ${agentId}. Check your email for login details.`,
        type: "success",
      });
      ...
      try {
        await supabase.functions.invoke("agent-approval-email", {
          body: { to: app.email, agentName: app.full_name, agentId, password, loginUrl: `${window.location.origin}/auth` },
        });
      } catch { /* optional */ }
  ...
  const rejectApplication = async (id: string, userId: string) => {
    ...
    await supabase.from("notifications").insert({
      user_id: userId, title: "Agent Application Update",
      message: "Your agent application was not approved at this time. Please contact support for more details.",
      type: "error",
    });

    await supabase.from("admin_activity_log").insert({
      admin_id: adminId, action: "reject_agent_application", entity_type: "user", entity_id: userId,
    });
  ...
  const saveEdit = async (userId: string) => {
    await supabase.from("profiles").update({
      full_name: editForm.full_name, phone: editForm.phone || null, city: editForm.city || null, bio: editForm.bio || null,
    }).eq("user_id", userId);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 border-b border-border pb-3">
        <button onClick={() => setActiveTab("agents")} className={`px-4 py-2 rounded-xl text-sm font-medium ${activeTab === "agents" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
          Agents ({agents.length})
        </button>
        <button onClick={() => setActiveTab("applications")} className={`px-4 py-2 rounded-xl text-sm font-medium relative ${activeTab === "applications" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
          Applications ({pendingApps.length})
          {pendingApps.length > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[10px] rounded-full flex items-center justify-center">{pendingApps.length}</span>}
        </button>
      </div>
            ...
            <div className="space-y-3">
              {applications.map(app => (
                <div key={app.id} className={`bg-card rounded-2xl border p-5 ${app.status === "pending" ? "border-amber-500/30" : "border-border"}`}>
                  <div className="flex items-start gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${app.status === "pending" ? "bg-amber-500/10" : app.status === "approved" ? "bg-emerald-500/10" : "bg-red-500/10"}`}>
                      {app.status === "pending" ? <Clock className="w-5 h-5 text-amber-500" /> : app.status === "approved" ? <CheckCircle className="w-5 h-5 text-emerald-500" /> : <XCircle className="w-5 h-5 text-red-500" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h4 className="font-display font-bold text-sm">{app.full_name}</h4>
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${app.status === "pending" ? "bg-amber-500/10 text-amber-600 border border-amber-500/20" : app.status === "approved" ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : "bg-red-500/10 text-red-600 border border-red-500/20"}`}>{app.status}</span>
                      </div>
                    ...
                    {app.status === "pending" && (
                      <div className="flex gap-2 flex-shrink-0">
                        ...
                        <button onClick={() => rejectApplication(app.id, app.user_id)} className="px-3 py-1.5 rounded-xl text-xs font-medium bg-red-500/10 text-red-600 border border-red-500/20 hover:bg-red-500/20 flex items-center gap-1">
